...
(function ($) {
  ...
  function loadData() {
    ...
    $.getJSON($.toFullPath('/api/activity/queryMyActivityLottery'), data,
        function (response) {
          var pageCount = 0;
          var items = response.data,
              keys = ['num', 'activityRecordName','activityRecordTitle','count', 'createDatetime','statusDesc', 'auditInfo','auditDate','operaction'],
              tbodyOptions = {
                preHandler: function (item) {
                  ...
                  switch (item.status) {
                    ...
                    case "1":
                      ...
                      if(item.activityRecordName && item.activityRecordName.indexOf("转盘抽奖")>-1){
                        operaction = '<a href="/views/activity/zhuanpan.html" target="_blank" class="text-primary" role="button">抽奖</a>';
                      }
                      if(item.activityRecordName && item.activityRecordName.indexOf("红包雨")>-1){
                        operaction = '<a href="/views/activity/hongbao.html" target="_blank" class="text-primary" role="button">抽奖</a>';
                      }
                      break;
                    case "2":
                      item.statusDesc = '已使用';
