...
(function ($) {
     ...
     *
     * i18n files are named <name>.js, or <name>_<languages>.js or <name>_<languages>_<country>.js
     * Where:
     *      The <languages> argument is a valid ISO Language Code. These codes are the lower-case,
     ...
     * @param  languages    (string, optional) languages/country code (eg, 'en', 'en_US', 'pt_BR'). if not specified, languages reported by the browser will be used instead.
    ...
    $.i18n.properties = function (settings) {
        ...
        // Try to ensure that we have at a least a two letter languages code
        ...
        // A locale is at least a languages code which means at least two files per name. If
        ...
        files.forEach(function (file) {
            ...
            // 2. with languages code (eg, Messages_pt.properties)
            var shortCode = settings.language.substring(0, 2);
            shortFileName = settings.path + file + '_' + shortCode + '.properties';
            // 3. with languages code and country code (eg, Messages_pt_BR.properties)
    ...
    /** Ensure languages code is in the format aa_AA. */
    $.i18n.normaliseLanguageCode = function (settings) {

        var lang = settings.language;
        if (!lang || lang.length < 2) {
            if (settings.debug) debug('No languages supplied. Pulling it from the browser ...');
        ...
        lang = lang.replace(/-/,"_"); // some browsers report languages as en-US instead of en_US
        if (lang.length > 3) {
